...
def load_gridsfm_case(json_path):
    ...
    gen_array = np.zeros((len(gens),10))
    ...
    branch_array = np.zeros((len(branches),13))
    ...
    gencost_array = np.zeros((len(gencost),7))
    for i,gen in enumerate (gencost):
    ...
    dcline_array = np.zeros((len(dclines),17))
